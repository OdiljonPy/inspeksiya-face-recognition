...
- Распознавание лиц: авто-галерея `person_XXXX` (open-set). `match_threshold=0.5` (тот же ID),
  ...
  Матч существующего ID подтверждается 2 кадрами (`match_confirm_frames`), кроме сильного
  (score>=`match_strong_score`=0.6). Серая зона (0.3..0.5) помечается `uncertain=1` в events.
  Det-гейт нового ID АДАПТИВНЫЙ: мелким лицам det>=0.70 (ложные детекции — земля/тени/
  одежда — почти всегда дают det 0.45-0.65), крупным (>=100px) достаточно 0.60. Резкость
  (blur) в гейте меряется по ТЕСНОМУ bbox лица (кроп с полями занижал дисперсию Лапласиана).
  **Best-shot**: фото галереи НЕ фиксируется первым кадром — при уверенном матче
  (score>=0.5) заметно лучший кадр (shot_quality: det×yaw×px×blur, гистерезис +15%)
  перезаписывает person_XXXX.jpg (gallery.maybe_update_crop). «Опущенная голова» в
  галерее сама заменяется анфасом, когда человек смотрит в камеру. ВАЖНО: наклон головы
  вниз (pitch) из 5 kps НЕ детектируется надёжно (проверено на реальных кадрах) — поэтому
  гейтить его порогом нельзя, лечится только best-shot'ом.
  Ракурсы в галерею добавляются ТОЛЬКО качественные и похожие на СВОЙ ID (own_score>=0.5) —
  защита от «отравления» галереи и слипания ID (см. gallery.maybe_add_embedding).
- ANPR: **тело** номера читается надёжно; регион восстанавливается ДВУМЯ механизмами:
  ...
  На тестовых данных (data/anpr_test + _cam_plate.mp4) 8/8 валидных номеров.
  Форматы РУз: частный (A123BC), юрлица (123ABC), прицепы/грузовые (1234AB).
