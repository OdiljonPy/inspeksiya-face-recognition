...
- Распознавание лиц: авто-галерея `person_XXXX` (open-set). `match_threshold=0.5` (тот же ID),
  ...
  + unsharp mask (gallery._enhance_gallery_crop) — только СОХРАНЕНИЕ фото,
  логика распознавания не тронута.
  **Фото и пороги (06.08.2026, ЯВНЫЙ запрос пользователя, БЕЗ флага — активно
  сразу после pull):** 1) фото галереи пишутся JPEG 100 (было 97);
  2) BEST-SHOT ВОЗВРАЩЁН в новом виде: Identity.photo_q хранит качество
  текущего фото (det × frontality × норм. blur); gallery.maybe_update_photo
  заменяет фото, когда трекер видит фронтальный кадр ЗАМЕТНО чётче
  (q > photo_q × 1.25 — гистерезис от постоянных перезаписей; known не
  трогаются — у них портрет с API; вызовы в tracker.py ветки 1 и 2, только
  фото — labels/score не меняются); 3) пороги подняты: recognition.
  min_det_score 0.4 -> 0.5, gallery.new_id_min_det_score 0.45 -> 0.60
  (реальные лица дают det >= 0.8, мусор 0.4-0.6). photo_q — числовой Python
  float в meta.json (np.float32 ломал json.dump — уже исправлено).
  eval после изменений: оба режима эталонные (клипы det >= 0.8 — пороги
  их не задевают).
...
- **track_enroll — отложенное создание ID по треку (ШАГ 1, 06.08.2026, ФЛАГ ВЫКЛЮЧЕН)**:
  `gallery.track_enroll` в settings.yaml (enabled: false = поведение старое,
  байт-в-байт; на проде можно включать только после проверки на живой камере).
  Включённый режим: кандидат-кадры трека копятся в буфер (te_* поля _Track),
  ID создаётся из ВЗВЕШЕННОГО СРЕДНЕГО эмбеддингов top-K лучших кадров
  (вес = det × frontality × норм. blur; агрегат давит пер-кадровый шум мелких
  лиц ~sqrt(K)), фото галереи — лучший кадр трека. Коммит: набрали topk кадров |
  таймаут max_wait_seconds | смерть трека (если кадров >= min_frames; иначе ID
  НЕ создаётся). Путь МАТЧИНГА не менялся. eval_quality: оба режима — known
  1 ID/0 новых на проходе 2, unknown 5 ID/0 новых; ANPR 7/7.
  **Шаги 2–3 тоже реализованы (06.08.2026), под тем же флагом:**
  шаг 2 — гейт самосогласованности буфера (`min_consistency: 0.35` — средний
  попарный cosine top-K кадров; текстуры дают случайные эмбеддинги 0..0.2 и
  ID не создаётся; свой человек >0.3; если реальные люди не заводятся —
  снизить до 0.30). Шаг 3 — жизненный цикл: ID из track_enroll рождается
  provisional (поле Identity, старый meta.json совместим — дефолт False;
  ВАЖНО при деплое: обновлять оба сервиса вместе, старый gallery.py отбросит
  поле при save); подтверждается, когда его матчит ДРУГОЙ трек не раньше
  confirm_min_gap_seconds (300) после создания (дробление трека не считается);
  неподтверждённые дольше provisional_ttl_days (7) удаляются GC вместе с
  событиями (EventLog.delete_person, зеркалит DELETE /api/gallery). GC гоняется
  через maintenance-хук InferenceWorker В ТОМ ЖЕ inference-потоке каждые
  gc_interval_hours (6) — галерея НЕ рассчитана на второй поток в процессе,
  НЕ выносить GC в отдельный thread.
  **Шаг 4 тоже реализован (06.08.2026), под ОТДЕЛЬНЫМ под-флагом**
  `match_by_aggregate: false` — матчинг неопознанного трека по скользящему
  взвешенному агрегату его последних match_agg_frames (5) кадров вместо
  одиночного эмбеддинга. Единственный шаг, трогающий путь матчинга (именно
  там был откат 13.07) — поэтому включать его на живой камере ОТДЕЛЬНО,
  после того как enabled:true (шаги 1-3) проверен. eval face-only: все три
  режима (off / 1-3 / 1-4) — эталонные ID, ноль новых на проходе 2.
  **Бенчмарк мелких лиц (scripts/eval_smallface.py, 06.08.2026)** — сценарий
  прода (работники заведены хорошими фото, камера видит мелко), LFW + деградация
  «камера» (JPEG q35 + смаз): при лице ~37px одиночный кадр узнаёт 71.5%,
  АГРЕГАТ 5 кадров (шаг 4) — 100%, FAR 0; при ~26px — 16.4% vs 76.4% (предел).
  Согласованность кадров своего человека p10=0.41 при 37px → гейт шага 2 (0.35)
  своих не режет; на 26px режет — и правильно. Чистый даунскейл БЕЗ компрессии
  почти не вредит (98% @26px) — узнаваемость убивает компрессия+смаз, поэтому
  битрейт/основной поток RTSP важнее всего (подтверждает диагноз июля).
  Замер save() галереи на 500 ID/2000 эмб.: med 8ms — НЕ узкое место,
  дебаунс не нужен.
- **По внешнему ТЗ (06.08.2026, «quality gate + устранение дублей»)** — БОЛЬШАЯ
  часть ТЗ уже была реализована track_enroll'ом (см. docs/pipeline_map.md);
  пороги ТЗ det>=0.65/bbox>=80px НЕ применены — это в точности июльский провал
  на обзорных камерах (лица 30-45px). Добавлено новое:
  1) **scripts/dedup_gallery.py** — offline-слияние дублей person_XXXX:
     центроиды -> union-find по cosine >= --merge-threshold (0.6) -> канонический
     = known либо самый старый; эмбеддинги переносятся, события перепривязываются,
     дубль удаляется. Dry-run по умолчанию с HTML-отчётом (фото рядом,
     data/dedup_report.html); --apply ТОЛЬКО при остановленном face-recognition;
     кластеры с known сливаются только с --include-known (person -> known).
  2) **margin-check** (`gallery.match_min_margin`, деф. 0.0=ВЫКЛ, ТЗ рекомендует
     0.08): матч не биндится, если отрыв топ-1 от ближайшего ДРУГОГО человека
     мал (gallery.identify2) — кадр уходит в серую зону.
  3) **pose_score** (gallery.py): yaw+pitch+roll по 5 точкам — заменил чистый
     yaw в ВЕСЕ КАДРА для фото (_te_weight: best-shot не зафиксируется на
     «смотрит вниз/вверх/набок»); пороги распознавания НЕ трогает.
  4) Доки: docs/pipeline_map.md (карта файл->роль), docs/identity_pipeline.md
     (state machine решения + калибровка порогов + порядок включения флагов).
