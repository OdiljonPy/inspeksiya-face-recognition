...
- Распознавание лиц: авто-галерея `person_XXXX` (open-set). `match_threshold=0.5` (тот же ID),
  ...
  Det-гейт нового ID АДАПТИВНЫЙ: мелким лицам det>=0.80 (ложные детекции почти всегда
  мелкие), крупным (>=100px) достаточно 0.72. Резкость (blur) в гейте меряется по
  ТЕСНОМУ bbox лица (кроп с полями занижал дисперсию Лапласиана и резал живых людей).
...
- ANPR: **тело** номера читается надёжно; регион восстанавливается ДВУМЯ механизмами:
  ...
  2) `region_ocr.py` (rapidocr, только для событий с region_uncertain) — кроп левой части
     номера, перебор долей 0.25/0.32/0.40, апскейл ×4, text_score=0.3.
