...
- **track_enroll — отложенное создание ID по треку (ШАГ 1, 06.08.2026, ФЛАГ ВЫКЛЮЧЕН)**:
  ...
  НЕ создаётся). Путь МАТЧИНГА и формат meta.json НЕ менялись (add_new_from_track
  создаёт обычный Identity). eval_quality: оба режима — known 1 ID/0 новых на
  проходе 2, unknown 5 ID/0 новых; ANPR 7/7. План дальше (по одному, после
  проверки пользователем): шаг 2 — гейт самосогласованности буфера (средний
  попарный cosine, убивает мусорные ID на текстурах); шаг 3 — provisional/
  confirmed + GC неподтверждённых; шаг 4 — матчинг по агрегату трека.
