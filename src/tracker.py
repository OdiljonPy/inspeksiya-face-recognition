...
class _Track:
    ...
    def __init__(self, bbox):
        ...
        self.match_label = None    # кандидат-СУЩЕСТВУЮЩИЙ ID (ждёт подтверждения)
        self.match_frames = 0      # сколько подряд кадров матч указывает на match_label
...
class CameraTracker:
    def __init__(self, gallery: Gallery, cfg: dict):
        ...
        # подтверждение существующего ID (одиночный кадр на мелких лицах шумит)
        self.match_confirm = int(gg.get("match_confirm_frames", 2))
        self.match_strong = float(gg.get("match_strong_score", 0.60))
    ...
    def _decide(self, t: _Track, f, frame, ts):
        ...
        def fr(label, score, is_new, crop, uncertain=False):
            if q is None:
                return FaceResult(t.bbox, label, score, is_new, crop, uncertain=uncertain)
            return FaceResult(t.bbox, label, score, is_new, crop,
                              q.det_score, q.width_px, q.blur, q.yaw_asym,
                              uncertain=uncertain)

        # общий гейт качества кадра (тот же, что для нового ID; учитывает ресайз)
        def quality_ok():
            return self.g.quality_ok_for_new(det, t.bbox, f.kps, frame, self._scale)
        ...
        if t.label is not None:
            ...
            if own is None:
                ...
                # score — похожесть на СВОЙ ID; ракурс добавится только если кадр
        ...
        if ident is not None and score >= self.g.match_threshold:
            # Сильный матч закрепляем сразу; обычный — после match_confirm_frames
            # подряд кадров на ОДИН и тот же ID (одиночный кадр на 30–45px шумит).
            if score < self.match_strong:
                if t.match_label == ident.label:
                    t.match_frames += 1
                else:
                    t.match_label = ident.label
                    t.match_frames = 1
                if t.match_frames < self.match_confirm:
                    return None          # ждём подтверждения — не мигаем чужим ID
            t.label = ident.label
        ...
        # 4) «Серая зона»: НЕ заводим новый ID. Если рядом есть существующий —
        # отдаём его, но помечаем uncertain (в событиях/API можно фильтровать).
        if ident is not None and score >= self.g.new_id_threshold:
            return fr(ident.label, score, False, ident.crop_path, uncertain=True)
        return None
