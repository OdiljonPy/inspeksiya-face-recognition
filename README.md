...
**Тип владельца и сверка с налогом (18.07.2026):** каждое событие транспорта получает
`owner_type` — `shaxsiy` (физлицо, номер «01 A 123 BC») / `yuridik` (юрлицо,
«01 123 ABC») / `kompaniya` (ИНН владельца из ГАИ совпадает с генподрядчиком объекта).
Базовый тип — по формату номера сразу; фоновая проверка ГАИ уточняет его и пишет
`owner_inn`, затем сверка с налогом заполняет `has_contract` (были ли фактуры
владелец → заказчик/генподрядчик объекта). Фильтры «Владелец»/«Договор» — на вкладке
Транспорт и в API; счётчики машин по типам — `GET /api/v1/vehicles/stats`
(см. [docs/API_INTEGRATION.md](docs/API_INTEGRATION.md)). Старые события:
`python scripts/backfill_owner_contract.py` (на сервере; `--no-gai` — только по формату).
...
## Известные люди (модуль known faces, 18.07.2026)
Внешняя платформа заводит работников по фото: `POST /api/v1/known-faces`
(ФИО + фото base64 + object_index). Человек попадает в общую галерею с меткой
`known_XXXX` и узнаётся камерами обычной логикой — события пишутся с его меткой
(в API и дашборде видно ФИО), счётчик появлений доступен в
`GET /api/v1/known-faces`. Незнакомые люди, как и раньше, получают авто-ID
`person_XXXX`. В дашборде — вкладка **«Известные»** (список + загрузка фото +
удаление). Дока — [docs/API_INTEGRATION.md](docs/API_INTEGRATION.md), раздел API 3.
